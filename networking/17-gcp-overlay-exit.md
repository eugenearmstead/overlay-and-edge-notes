...
This exit is **backup only**. MUST NOT publish a ranked “try exit A then B” ladder in a public spec. MUST NOT select this exit on the **agent workstation** to reproduce (forthcoming OEN-S18).
...
2. **P2 — Advertise both default routes and keep accept-dns off on the exit VM.**
   - **Action:** On `<exit-node>`, advertise exit plus `0.0.0.0/0,::/0`. Set `accept-dns=false` on this VM (guest DNS pretty-print is not PeerAPI; turning accept-dns on can break Recursion Desired (RD)=0 exit DNS — forthcoming OEN-23).
...
3. **P3 — Fix overlay UDP PORT and the cloud firewall together.**
   ...
   - **On failure:** If PORT was 0, the VPC allow did nothing. Set PORT, restart, then re-check. Forthcoming OEN-S23.
...
5. **P5 — Soak from a LAN Pi, not from the agent workstation.**
   ...
   - **On failure:** If v4 works and v6 leaks home, that may be LAN unique-local address (ULA) Happy-Eyeballs (forthcoming OEN-20) or Wi-Fi vs cellular underlay (forthcoming OEN-S26) — not automatically a GCP NAT bug. If cellular-class clients work and Wi-Fi does not, do not keep changing GCP Maximum Transmission Unit (MTU).
...
7. **P7 — Ignore misleading DNS and leak-test canaries.**
   ...
   - **Expected:** Exit DNS is proven with PeerAPI DoH from a **non-exit** client (forthcoming OEN-05 / OEN-14), not with guest `dig`.
...
- [OEN-01 Overlay SSH byte cliff](01-overlay-ssh-byte-cliff.md) (built)
- [OEN-16 Commercial WireGuard endpoint rotation](16-commercial-wg-endpoint-rotation.md) (built) — underlay to this VM may still be commercial WireGuard (WG) from home
- OEN-13 overlay vs underlay vs unique-local layers (forthcoming)
- OEN-23 accept-dns on a cloud exit (forthcoming)
- OEN-S18 do not set exit on the agent workstation (forthcoming)
- OEN-S21 cloud exit hairpins home LAN (forthcoming)
- OEN-S22 Identity-Aware Proxy vs overlay key exchange (forthcoming)
- OEN-S23 VPC 41641 while PORT=0 (forthcoming)
